...
function ok(msg: string)   { console.log(`${GREEN}  ✅ PASS${RESET} — ${msg}`); passed++; }
...
async function testPaymentsIsolation(centerAId: number, studentAId: number) {
...
async function testCoursesIsolation(centerAId: number, centerBId: number) {
  ...
  if (!student) { fail("Talaba topilmadi!"); return; }

  ok(`Jami kurslar (filtrsiz): ${student.courses.length} ta`);

  // THE FIX: filter after fetch
  const centerCourses = student.courses.filter((c: any) => c.centerId === centerAId);
  const leakedCourses = student.courses.filter((c: any) => c.centerId !== centerAId);

  if (leakedCourses.length > 0) {
    ok(`Eski kod muammoli edi: filtrsiz ${student.courses.length} ta kurs — boshqa markaz kurslari: ${leakedCourses.map((c: any) => c.name).join(", ")}`);
  }

  if (centerCourses.some((c: any) => c.centerId !== centerAId)) {
    fail(`KURS IZOLYATSIYASI BUZILDI! Boshqa markazning kursi ko'rinmoqda: ${leakedCourses.map((c: any) => c.name).join(", ")}`);
  } else {
  ...
  const hasCenterACourse = centerCourses.some((c: any) => c.name === "TEST_ISOLATION_CourseA");
  if (hasCenterACourse) {
  ...
  const hasCenterBCourse = centerCourses.some((c: any) => c.name === "TEST_ISOLATION_CourseB");
  if (hasCenterBCourse) {
...
async function testAttendanceIsolation(centerAId: number) {
  ...
  const leakedAttendance = student.attendance.filter((a: any) => a.centerId !== centerAId);
  if (leakedAttendance.length > 0) {
    fail(`DAVOMAT IZOLYATSIYASI BUZILDI! Markaz B davomati ko'rinmoqda: ${leakedAttendance.map((a: any) => `id=${a.id}`).join(", ")}`);
  } else {
    ok(`Faqat markaz A davomati ko'rinmoqda`);
  }

  // Check: centerA's attendance is present (PRESENT status)
  const hasPresentRecord = student.attendance.some((a: any) => a.status === "PRESENT" && a.centerId === centerAId);
  if (hasPresentRecord) {
  ...
  const wouldHaveLeaked = brokenAttendance?.attendance.some((a: any) => a.centerId !== centerAId);
  if (wouldHaveLeaked) {
...
async function testStudentLookupIsolation(centerAId: number, centerBId: number) {
...
// ─── CLEANUP ─────────────────────────────────────────────────
async function cleanup(centerAId: number, centerBId: number) {
